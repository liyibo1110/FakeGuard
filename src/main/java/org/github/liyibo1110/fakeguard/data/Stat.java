...
public class Stat implements Record {
	...
	private int numChildren;	// 多少个子节点
	...
	public Stat(long czxid, long mzxid, long ctime, long mtime, int version,
			int cversion, int aversion, long ephemeralOwner, int dataLength, int numChildren,
			long pzxid) {
	...
	public void serialize(OutputArchive archive, String tag) throws IOException {
		...
		archive.writeInt(dataLength, "dataLength");
		archive.writeInt(numChildren, "numChildren");
	...
	public void deserialize(InputArchive archive, String tag) throws IOException {
	    ...
	    dataLength = archive.readInt("dataLength");
	    numChildren = archive.readInt("numChildren");
	...
	public int compareTo(Object _obj) throws ClassCastException {
		...
		if (ret != 0) return ret;
		ret = (dataLength == obj.dataLength) ? 0 : ((dataLength < obj.dataLength) ? -1 : 1);
		if (ret != 0) return ret;
		ret = (numChildren == obj.numChildren) ? 0 : ((numChildren < obj.numChildren) ? -1 : 1);
		if (ret != 0) return ret;
	...
	public boolean equals(Object _obj) {
		if (!(_obj instanceof Stat)) return false;
	    ...
	    if (!ret) return ret;
	    ret = (numChildren == obj.numChildren);
	    if (!ret) return ret;
	    ret = (pzxid == obj.pzxid);
	    if (!ret) return ret;
		return ret;
